  ...
  try {
    ...
    if (
      ...
      if (
        ...
        html.includes("ltx_page_main") ||
        html.includes("ltx_abstract")
      ) {
...
async function getRecentAIPapers(): Promise<string> {
  try {
    ...
    throw new Error(
      `获取最新论文失败: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
...
function getArxivPdfUrl(input: string): string {
  try {
    ...
    throw new Error(
      `获取PDF链接失败: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
...
async function parsePaperContent(
  input: string,
  paperInfo?: any
): Promise<{ content: string; source: "html" | "pdf" }> {
  ...
  try {
    ...
    if (paperInfo) {
      ...
      if (paperInfo.authors && paperInfo.authors.length > 0) {
        outputContent += `作者: ${paperInfo.authors
    ...
    throw new Error(
      `论文内容解析失败: ${error instanceof Error ? error.message : String(error)}`
    );
  } finally {
    // 清理临时 PDF 文件
        ...
        inputSchema: {
          ...
          properties: {
            input: {
              type: "string",
              description:
                "arXiv 论文URL（如：http://arxiv.org/abs/2403.15137v1）或 arXiv ID（如：2403.15137v1）",
            },
          },
          required: ["input"],
        },
      ...
      case "search_arxiv": {
        ...
        return {
          content: [
            {
              type: "text",
              text: `找到 ${results.papers.length} 篇相关论文（总计 ${
